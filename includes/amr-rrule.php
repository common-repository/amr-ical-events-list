...
/* --------------------------------------------------------------------------------------------------- */
function print_date_array1($datea) { // note the undderscores are just to pad out so dates appear under each other
...
global $amr_globaltz;
	...
	$dateobj = amr_newDateTime();
	foreach ($datearray as $i=> $datea) {
		$d = $datea;       //echo '<br />parts day: ';var_dump($d);
		$d['day'] = '1'; //20180818 changed from integer 1 to string 1 in case that was causing the unrecreateable problem
		$day1 = amr_create_date_from_parts ($d, $tz); 
		if (is_object ($day1) ) { // 20180818  changed logic to avoid boolean returned.
			...
			if (isset($_GET['rdebug'])) echo '<br/> Skipping - Non object date generated from '. print_r($d, true);
		}
	}
	if (isset($_GET['rdebug'])) echo '<hr>';
	if (!empty($newdatearray)) return ($newdatearray);
...
function amr_limit_by_day_of_week (&$datearray, $pby, $tz) {
	...
	if (empty($datearray)) return;
	if (isset($_GET['rdebug'])) { echo '<br>Limit by day of week <pre>'; print_r($pby); echo '</pre>'; var_dump($datearray);}
	foreach ($datearray as $i=> $d) {
		$dateobj = amr_create_date_from_parts ($d, $tz);
		if (is_object ($dateobj) ) {
			$w = $dateobj->format('w');
			...
			if (!isset($pby['BYDAY'][$w]) ) { // 20181002 was not checking BYDAY !!
...
function amr_process_RRULE($p, $start, $astart, $aend, $limit )  {
	...
	//echo  '<br />rule passed'; var_dump($p);
	...
	if (empty($int)) var_dump($p); //201807
