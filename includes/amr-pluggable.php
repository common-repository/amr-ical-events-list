	...
	function amr_ical_showmap ($text) { /* the address text */
		global $amr_options;
			$t1 = __('Show in Google map','amr-ical-events-list');
				...
				$t3 = str_replace(array( '\r', '\n'), '', $text);  //201705 test data had /n - bugon their side?
			else 
...
//		echo 'link='.$link.'<br/> with special chars: '.htmlspecialchars($link); yikes debug left in
	return ('<form method="GET" action="'.htmlspecialchars($link).'">'  //20151125 - some folks systems don't do $_REQUEST properly
...
// check for  title, type = audio/... video/.. etc
				if ($typeparts[0] === 'image') {  // only include if NOT already in the content
					...
					if (!empty($item['thumb'])) {  // only do thumb?
						$item_title = PHP_EOL.'<img alt="'.$item_title
						.'" src="'.esc_url($item['thumb']).'" />';
					}
					else $item_title = PHP_EOL.'<img alt="'.$item_title
...
//2017083 for &amp;	.htmlspecialchars($p['X-WR-CALNAME'])
					.$p['X-WR-CALNAME']
					.'</a><!-- '.$desc.' -->';
			}
			...
			$p['icsurl'] = // dont add PHP_EOL - sometimes makes wp add <br> tags
	...
	//				.'#'.$e['Bookmark']
					...
					if (!empty($amr_options['listtypes'][$amr_listtype]['general']['Default Event URL'])) {
						$e_url = ' class="url" href="'
							.esc_url($amr_options['listtypes'][$amr_listtype]['general']['Default Event URL']).'" ';
						}
...
if (!function_exists('add_cal_to_google')) {
	...
	/* adds a button to add the current calemdar link to the users google calendar */
		...
		else // dont add PHP_EOL - sometimes makes wp add <br> tags
...
if (!function_exists('add_event_to_google')) {
	...
	// dont add PHP_EOL - sometimes makes wp add <br> tags
...
function amr_list_events($events,  $tid, $class, $show_views=true) {
	...
	if (empty ($events)) $events = array();  //2020022
...
if (!function_exists('amr_format_value')) {
	...
	*/
	...
	//					.amr_format_date ('l jS F Y, H:i e ', $content).'">'
				...
				case 'EndDate': { // but is not an object so won't get here
				...
				case 'EndDate':
				case 'UNTIL': { //20210915 where used?  NOT EndDate in eventinfo is being passed as a atring
...
function amr_wp_format_date( $format, $datestamp) { /* 20210915 date object , no longer do gmt option */
	$tz = $datestamp->getTimezone();
	return (wp_date($format, $datestamp->format('U'), $tz ));
	}
}
...
global 	$amr_globaltz;
	...
	if (isset($_GET['debugtz'])) echo  '<br />Time='.$time;
...
global 	$amr_options,
	...
	if (isset($_GET['debugtz'])) echo  '<br />After tz set: '.$datestamp->format('c e');

	if ($method === 'wp') { 
		return amr_wp_format_date ( $format, $datestamp, false);
	}
	else if ($method === 'wpgmt') { if (isset($_GET['debugtz'])) echo ' Using wpgmt ';
		return amr_wp_format_date ( $format, $datestamp, true);
	}
	else if ($method === 'amr') { if (isset($_GET['debugtz'])) echo ' Using amr_date_i18n ';
		return amr_date_i18n ( $format, $datestamp);
	}
	else {
		if (stristr($format, '%') ) {
			if (isset($_GET['debugtz'])) echo ' using % ?';
			return (strftime( $format, $datestamp->format('U') ));  /* keep this for compatibility!  will not localise though */
		}
		else {
			if (isset($_GET['debugtz'])) echo ' Using passed format ';
...
global $amr_globaltz, $amr_options;
	...
	if (isset ($amr_options['no_images']) and $amr_options['no_images']) 
		$t3 = $text1;
	else  // sadly don;t add PHP_EOL in case wp addss a <br> when we don't want it
...
if (!function_exists('amr_format_mailto')) { //201710
	...
	return ('<a class="email" title="'.$title.'" href="mailto:'.$mailto.'">'.$text.'</a>');
	}
}
...
if (!function_exists('amr_format_organiser')) {
	...
	If (ICAL_EVENTS_DEBUG) {echo '<br />Organiser array:    '; var_dump($org);}
		...
		if (!empty ($org['CN'])) {    
			...
			//'<a href="mailto:'.$org['MAILTO'].'" >'.$org['CN'].'</a>';
			else 
				$text = $org['CN'];
		}
		else {
			if (!empty  ($org['MAILTO'])) 
				...
				$title=__('Organiser', 'amr-ical-events-list'));
				//'<a href="mailto:'.$org['MAILTO'].'" >'.$org['MAILTO'].'</a>';
		}
		if (!empty ($text)) $text .= '&nbsp;';
...
if (!function_exists('amr_format_attendees') ) { // overwritten by addon
	...
	if (is_array($attendees)) {
		...
		if (!empty($list)) { //*** a list would be more semantic
			//$text .= '<ul><li>'.implode ('</li><li> ',$list).'</li></ul>';
			$text = implode (PHP_EOL.'<br />',$list);
		}	
...
if (!function_exists('amr_format_PARTSTAT') ) { // called from format attendee
	function amr_format_PARTSTAT ($participation_status) {
		...
		switch ($participation_status) {
			case 'ACCEPTED': $text = ' &radic; '.$text ; break;
			case 'DECLINED': $text =  ' &#10008; '. $text; break;
			case 'TENTATIVE': $text =  ' &#63; '. $text; break;
			case 'DELEGATED': $text = ' &rarr; '. $text ; break;
			case 'NEEDS-ACTION': $text = ' &#33; '. $text ; break;
			}
		return ('<span class="particpation '.$participation_status.'">'.$text.'<span>');	
	}
}
