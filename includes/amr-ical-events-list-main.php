...
/* Dropping support for php4 see http://acko.net/blog/php-clone */
...
function amr_get_css_url_choices () { // only called from admin screen
	$dir = amr_get_css_path();
	$css_url = ICAL_EVENTS_CSS_URL;  // from users upload css

	$files = amr_get_files($dir, '.css'); 

	if (!empty($files)) {
		foreach ($files as $i => $f) {
			$files[$i] = ICAL_EVENTS_CSS_URL.$f;
		}
	}	
	$files[] = 	ICALLISTPLUGINURL.'css/icallist2021.css';
	$files[] = 	ICALLISTPLUGINURL.'css/icallist.css';
	$files[] = 	ICALLISTPLUGINURL.'css/icallist2017.css';
	return($files);
}
...
/* 20171010 - do not offer this custom css anymore - they should use other options		
		...
		if (!file_exists($css_file)) { // if there is no starting css file, then copy the plugin version over  
			...
			else amr_copy_file(ICALLISTPLUGINDIR.'css/icallist2017.css',$css_file );
		}
		return $css_path;
*/		
	}
...
global $amr_options;
global $amr_listtype;
	// 20171010 do not do here - don't need the dir // $icalstyledir = amr_get_css_path(); // create the custom css path if it exists ICALSTYLEURL;
	...
	else {  // we don't have stored options ?! just use defaults
		wp_register_style('amr-ical-events-list', $icalstyleurl, array( ), 1.0 , 'all' );
		wp_enqueue_style('amr-ical-events-list' );
		wp_register_style('amr-ical-events-list_print', ICALSTYLEPRINTURL, array(), 1.0 , 'print');
		wp_enqueue_style('amr-ical-events-list_print');
	}
	wp_enqueue_style('dashicons', 999); //20211117  dashicons font for icons etc
}
...
function DEL_amr_show_in_events_timezones (&$e) { // only change at last minute before display, 
//else you may mess up ordering and recurring calculations
global $amr_options;
	var_dump($amr_options);
	if (!empty($e['timezone'])) { // do we have an event timezone stored ?
		if (is_obj($e['timezone'])) {
			//$e[] //*** amr
		}
	}
...
//$e = amr_decide_display_timezone($e);  // amr *** rather put at each display point ?
...
function amr_derive_for_list_or_eventinfo (&$e) {  // should only derive the fields we need
	...
	if (amr_need_this_field('_map')) { 	// just prepare with whatever we have, leave format to later
...
function amr_sort_by_key($data, $key) { //201807 remove deprecated create function
...
/*		Echo '<br />ASTART: '; var_dump($astart);
		Echo '<br />EVENT : '; var_dump($eventdate);
		
		$dd = amr_daysDifference ($astart,$eventdate);  echo '<br />dd='.$dd;
		if ($astart > $eventdate) echo '<br />a is graeter er';
		else if ($astart < $eventdate)  echo '<br />a is less er';
		else	echo '<br/>a is less or = ';
*/		 
		...
		if (($eventdate <= $aend) and
			(($eventdate > $astart) or ($eventdate == $astart))) 
			return ( true);
		else 
			return (false);
}
...
//
		...
		//if (ICAL_EVENTS_DEBUG) {echo '<br />Constrained =';var_dump($constrained);}
...
function amr_process_icalevents($events, $l_start, $aend, $limit) {
		...
		foreach ($events as $i=> $event) {
			...
			if (ICAL_EVENTS_DEBUG) {echo ' + '.$i.' generated repeats = '; if ((is_array($more))) echo count($more); else echo '0';}
			if (is_array($more)) 
...
/* amr here is the main html  code  *** */
			...
			and empty($components) ) {
				...
				$thecal .= amr_list_events($components, $tid, $class, $show_views=true);
					if (ICAL_EVENTS_DEBUG) {
						echo '<br />Peak Memory after compiling listing :'.amr_memory_convert(memory_get_usage(true));
					}
			}
...
// now check groupings
	...
	if (!empty ($shortcode_params ) ) {
		...
		if (!empty($amr_limits['listtype'])) {
			...
			if (ICAL_EVENTS_DEBUG) echo '<br />listtype set to: '.$amr_listtype;
		}
	}
...
//-------------------------------
	...
	if (empty ($shortcode_params['start'])) {
		if (ICAL_EVENTS_DEBUG) echo '<br/>No shortcode start parameter, use "now"';  //20220107 
		$amr_limits['start'] = date_create('now',$amr_globaltz);	
		if (ICAL_EVENTS_DEBUG) echo ' - Start set to:'.$amr_limits['start']->format('c');  //20220107 		
	}
	else {
		if (ICAL_EVENTS_DEBUG) echo '<br/>shortcode start parameter found: '.$shortcode_params['start'];  //20220107 
...
// work out whether we are doing a calendar or not.  calendars must still respond to at least the month navigation
	...
	foreach ($amr_limits as $i => $a) {  // create our date objects
		if (($i === 'start') or ($i === 'end')) {  //echo 'limits';var_dump($amr_limits);
			if (!(is_object($a))) { //why not object?
				if (ICAL_EVENTS_DEBUG) echo '<br/>'.$i.' is not a date object yet.  Try convert '.$a;  //20220107 
				$ystr = intval(substr($a,0,4));/* year */ 
				$mstr = intval(substr($a,4,2));  /* month */
				$dstr = intval(substr($a,6,2));  /* day*/
				$dateok = checkdate($mstr, $dstr, $ystr);
				if ($dateok) {
					if (ICAL_EVENTS_DEBUG) echo '<br/>Checkdate succeeded ';
					$amr_limits[$i] = date_create($a,$amr_globaltz);
					if (ICAL_EVENTS_DEBUG) echo '<br/>'.$i.' now set to '.$amr_limits[$i]->format('c');
...
//	now find our end date  - may get overridden if calendar
	if (is_object($amr_limits['start'])) {
		...
		amr_tell_error (__('Error: Start data of range is not a date object. Please advise administrator','amr-ical-events-list'));
