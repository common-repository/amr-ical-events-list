...
if (!defined ('ICAL_EVENTS_DEBUG')) {
	if (isset($_GET["debug"])  
		...
		foreach (array('debugexc', 'rdebug', 'cdebug', 'tzdebug', 'debugall', 'debugq', 'memdebug', 'debugtime') as $d) {
			unset($_GET[$d]);
		}
	}
}
...
if (ICAL_EVENTS_DEBUG) {
	...
	echo '<br /> events version:'.AMR_ICAL_VERSION.' list version:'.AMR_ICAL_LIST_VERSION.' php:'.PHP_VERSION.' wp:'
	.get_bloginfo('version').' charset:'.get_option( 'blog_charset' ).' encoding:'.mb_internal_encoding();
	echo ' memlimit:'.ini_get("memory_limit");
	echo ' maxtime:'.ini_get('max_execution_time').' seconds';
	echo ' global tz:'.$amr_globaltz->getName();
	echo ' ical tz:'.$ical_timezone->getName();
}
...
define('ICALSTYLEURL', $url.'css/icallist2021.css');
define('ICALSTYLEFILE', $dir.'css/icallist2021.css');
...
function amr_default_fields_to_use () {
	...
	return (array (
			...
			'RESOURCES'=> 		0,
			'STATUS'=> 			0,
			'EventDate' => 		1, /* the instance of a repeating date */
			'StartTime' => 		1,
			'EndDate' => 		1,
			'EndTime' => 		1,
			'DTSTART'=> 		0,
...
//	else 	
	...
	$amr_calprop = array (
			...
			'addtogoogle' 	=> array('Column' => 0, 'Order' => 10, 'Before' => '', 'After' => ''),
	...
	$amr_compprop = array
		...
		'Descriptive' =>
	 	array_merge (
			...
			'map'=> 			array('Column' => 3, 'Order' => 42, 'Before' => '', 'After' => ''),
			'addevent' => 		array('Column' => 3, 'Order' => 1, 'Before' => '', 'After' => ''),
			'subscribeevent' => array('Column' => 3, 'Order' => 2, 'Before' => '', 'After' => ''),
			'subscribeseries' => array('Column' => 3, 'Order' => 3, 'Before' => '', 'After' => ''),
			'GEO'=> 			$dfalse,
	...
	//		'age'=> $dfalse,
			...
			'allday' => 	array('Column' => 0, 'Order' => 4, 'Before' => '&nbsp;', 'After' => ''),
			'timezone'=> 	$dfalse,
			'COMPLETED'=> 	$dfalse,
			'FREEBUSY'=> 	$dfalse,
			'TRANSP'=> 		$dfalse),
	...
	switch ($i)	{
		...
		case 5: {
			...
			$amr_options['listtypes'][$i]['compprop']['Descriptive']['map']
				= array('Column' => 1, 'Order' => 41,'Before' => '', 'After' => '');
			break;
			}
		case 6: {
...
function date_season ($type,$D) {
	...
	if (empty($type)) $type='Meteorological'; //20210611
...
function amr_getset_options ($reset=false) {
	...
	if ($reset) {
		...
		if (($d = delete_option('amr-ical-events-list')) or 
			($d = delete_option('AmRiCalEventList')) or 
			($d = delete_option('amr-event-fields-in-use')))
